...
def makestoredump(data):
    df_reviews = data
    df_s_group = df_reviews.groupby(["store_id"])
    series_over_3 = df_s_group.size()[df_s_group.size() > 10]
    series_over_3_store = series_over_3.to_frame().reset_index()['store_id']
    ...
    for i in series_over_3_store:
        for k in range(df_reviews[df_reviews['store_id'] == i].id.count()):
            store_list.append(i)
            user_list.append(
                df_reviews[df_reviews['store_id'] == i].iloc[k].user_id)
            score_list.append(
                df_reviews[df_reviews['store_id'] == i].iloc[k].score)
